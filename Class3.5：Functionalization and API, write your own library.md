...
3. `"C:\swigwin-4.3.0\Lib"`
4. 解壓縮到系統儲存程式的位置(可能是C槽之類的) 
5. 設定為環境變數(什麼是環境變數，讓terminal可以呼叫這個程式)
6. `swig --version` 確認安裝完成
7. 確定C編譯器也被設定為環境變數 `g++ --version`
8. 打包C++ function，編譯並輸出
9. 參考.h檔案內容寫一個C_main.cpp
10. 測試迭代速度差異
