...
ser = None  # Default to None to avoid errors later
...
def twos_complement(value, bits):
    """Convert value to Two's Complement binary string"""
    if value < 0:
        value = (1 << bits) + value
    return format(value, f"0{bits}b")

def ones_complement(value, bits):
    """Convert value to One's Complement binary string"""
    if value < 0:
        value = ~(-value)
    return format(value & ((1 << bits) - 1), f"0{bits}b")

def ieee_754_float(value):
    """Convert float to IEEE-754 32-bit binary string"""
...
while True:
    ...
    int_value = random.randint(-128, 127)  # 8-bit integer
    float_value = round(random.uniform(-10, 10), 2)  # IEEE-754 float
